...
struct do_if {
    ...
    template<
            is::bool_type cond,
            is::function THEN, typename THEN_arg_list,
            is::function ELSE, typename ELSE_arg_list> requires(Value(cond))
    struct apply_on<L(cond, THEN, THEN_arg_list, ELSE, ELSE_arg_list) > {
...
struct insert_l {
    ...
    template<typename T, is::arg Index, typename T1, typename... Ts> requires (
...
struct remove_l {
    ...
    template<is::arg Index, typename T1, typename... Ts> requires (
            RetV(is_greater, Index, A(0)) &&
...
struct replace_l {
    ...
    template<typename T, is::arg Index, is::list l> requires (
...
public:
    ...
    struct mask_list {
        ...
        template<typename V> requires(RetV(is_greater, N, A(0)))
        struct apply_on<V> {
            using ret = L(V, T);
        };
        ...
        template<is::list V> requires(RetV(is_greater, N, A(0)))
        struct apply_on<__1_arg<V>> {
            using ret = L(V, T);
        };
...
private:
    ...
    struct __trunc {
        ...
        template<is::list l> requires(RetV(is_greater, Len(l), A(2)))
        struct apply_on<l> {
            using ret = Ret(C(pop_back, pop_back), l);
        };

        template<is::list l> requires(RetV(is_value_equal, Len(l), A(2)))
        struct apply_on<l> {
            using ret = Null;
        };
    };
    ...
    private:
        ...
        public:
            ...
            template<is::list l> requires(RetV(is_same, Ret(peek_front, l), STOP))
            struct apply_on<l> {
                using ret = l;
            };
        };
